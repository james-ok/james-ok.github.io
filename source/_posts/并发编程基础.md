...
### 并发容器
* 早起线程安全的容器，例如：Vector、Hashtable虽然都是线程安全的，但是他是使用synchronized修饰在各个方法上来实现的，性能非常低。
* 通过Collections.synchronizedList()工具的方法虽然可以将一个非线程安全的ArrayList或者HashMap转换成一个线程安全的容器，但其实现方式任然是使用synchronized关键字使用同步代码块的方式到达
线程安全的目的，虽然不是加在方法上，但其性能也是非常的低。
* JDk1.5以后，Java现在并发容器类ConcurrentHashMap和CopyOnWriteArrayList

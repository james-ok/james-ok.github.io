...
### ThreadLocal
ThreadLocal使用场景
* 每个线程独享一个对象，例如每个线程中创建一个`SimpleDataFormat`对象，该方式使用`withInitial`来创建对象，因为`SimpleDataFormat`对象不受其他因素影响，PS：
```java
public static ThreadLocal<SimpleDateFormat> formatLocal = ThreadLocal.withInitial(() -> new SimpleDateFormat("yyyy-MM-dd"));
```
